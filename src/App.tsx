...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-navy-950 text-slate-100 selection:bg-civic-saffron-500 selection:text-white relative">
      {/* Header with Navigation & Language dropdown */}
      <Header
        activeTab={activeTab}
      ...
      {/* Floating Quick Action: Ask AI Assistant (when not on chat tab) */}
      {activeTab !== 'ask-ai' && (
        <button
        ...
        >
          <Sparkles className="w-4 h-4 text-navy-950 animate-spin [animation-duration:4s]" />
      ...
      <AuthProvider>
        <DemoProvider onNavigateTab={handleTabChange}>
          <div className="flex min-h-screen">
            <LeftRail activeTab={activeTab} setActiveTab={handleTabChange} />
          ...
          <ReadinessScoreCard />
          {/* Guided Tour (renders nothing if not active) */}
          <GuidedTour />
        </DemoProvider>
      </AuthProvider>
    </LanguageProvider>
